    ...
    if (user) {
      const fetchFavorites = async () => {
        ...
        if (userDoc.exists()) {
          const userData = userDoc.data();
          const favoriteIds = userData.favorites || [];
          const dislikedIds = userData.dislikes || [];
          const filteredFavoriteIds = favoriteIds.filter(id => !dislikedIds.includes(id));
          if (filteredFavoriteIds.length > 0) {
            const q = query(collection(db, 'recipes'), where('__name__', 'in', filteredFavoriteIds.map(String)));
            const querySnapshot = await getDocs(q);
            const favoriteRecipes = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setFavorites(favoriteRecipes);
          }
